...
mkValidator ts _ _ context
    ...
    where
      ...
      tokenCost :: Integer
      tokenCost = (tsCost ts) P.- royaltyPaymentCost
...
validator = Scripts.validatorScript (typedValidator ts)
    where ts = TokenSaleParams { tsSellerAddress  = pubKeyHashAddress "SELLER_PUB_KEY_HERE" -- update to new seller
                               , tsRoyaltyAddress = pubKeyHashAddress "ARTIST_PUB_KEY_HERE" -- get original seller
                               , tsCost           = TOTAL_COST                              -- Token Cost
                               , tsRoyalty        = ROYALTY_AMOUNT                          -- royalty  = 1 / % of returns
                               }
